...
const consultTypeBaseDao = require('../dao/productosDao')


const nuevoProducto = async (req,res)=>{
	const timestamp = Date.now()
	const {body:{nombre, descripcion, codigo, foto, precio, stock}} = req
	const database = await consultTypeBaseDao('mongo')
	const result = await database.saveProducto({timestamp, nombre, descripcion, codigo, foto, precio, stock})
	res.json(result)
}
